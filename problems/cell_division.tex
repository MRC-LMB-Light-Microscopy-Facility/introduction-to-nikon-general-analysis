...
\begin{enumerate}
    ...
    \item Segment the nuclei of each cell \endnote{Use \menu{Preprocessing>Background>Rolling Ball} with a radius of 10px followed by \menu{Segmentation>Threshold>Threshold} with intensity above 10, Smooth 8x, Clean 1x, FillHoles OFF, Separate 1x and Size[px] above 5px.}
    ...
    \item Track the cells over time. Use \menu{Tracking > 2D Tracking > Track Particles} with ``Time Column'' set to "TimeLapseIndex" or "Time" and ``Maximum Speed'' set to INF. Add \menu{Tracking > Tracks > Accumulate \& Group}.
