...
\paragraph{Problem}
A developing sand dollar embryo expressing GFP-histone H2B was imaged using video-microscopy every \SI{30}{\second} for \SI{100}{\hour}. We want to segment and visualize the number of cells at each frame in a first step and later track the cells over time.

\paragraph{Dataset} The file \directory{7 dendraster excentricus/cil-15798.tif} has one channel and 200 time points.

\paragraph{Estimated Duration} 40 min \quad \textbf{Difficulty:} Intermediate
...
\paragraph{Reference} 
Von Dassow~\cite{vondassow2011}.


% \begin{description}
%     \item[Task:] Track the cells along the division and count the number of cells over time.
%     \item[Data:] ``cil-15798.tif'' with one channel and 200 time points.
...
\begin{enumerate}
    \item Open the image \directory{7 dendraster excentricus/cil-15798.tif} and open \menu{Image > new GA3 recipe}.
    ...
    \item However, this gives the count only for a single frame; to get the results for each frame, we can use \menu{Data Management > Accumulate Records}.
    \item Next, let's visualize the number of nuclei vs time using \menu{Results>Graphs>Linechart}.
    \item We could also track the individual nuclei by dropping \menu{Tracking>Track Objects} and \menu{Tracking>Tracks>Accumulate Tracks}.
\end{enumerate}
