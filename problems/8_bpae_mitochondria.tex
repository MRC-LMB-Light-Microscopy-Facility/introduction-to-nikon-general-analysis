...
\paragraph{Problem}
BPAE cells have been labelled with a mitochondrial marker and imaged in widefield microscopy. We would like to measure the ``amount'' of mitochondria in each cell. This time, we are going to use the General Analysis module instead of GA3. This module can be combined with the JOBS module to guide image acquisition.
...
\begin{enumerate}
    ...
    \item If not satisfied with the result, try combining a \menu{Region Growing>watershed} with an independent thresholding step by creating two binaries and combining them.
\end{enumerate}
...
\begin{enumerate}
    ...
    \item Add a \menu{Denoising > median} (count:1x), a \menu{Background Correction >
