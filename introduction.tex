...
\paragraph{Pearson correlation coefficient} The Pearson correlation coefficient (PCC)~\cite{pcc} is a statistical tool used to measure the linear correlation between two signals indicating the potential colocalization of two labelled molecular populations. If we have the pair of red and green intensities $(r_i,g_i)$ at each location $i$ in the image, the PCC is defined as
$$\mathrm{PCC} = \frac{\sum_i(r_i-\bar{r})(g_i-\bar{g})}{\sqrt{\sum_i(r_i-\bar{r})^2} \sqrt{\sum_i(g_i-\bar{g})^2}}$$
with the means $\bar{r} = n^{-1}\sum_i r_i$ and $\bar{g} = n^{-1}\sum_i g_i$. The value of the PCC ranges from -1 to 1 where a value of 1 indicates a perfect positive linear correlation.

\paragraph{Table join} To combine measurements from two tables, we can use a ``join''~\cite{join} which will find matching rows sharing a common reference index. The ``inner join'' operation will include only the matching rows in both tables (See Figure~\ref{fig:join}). Manipulating tables can replace logical operations on binary masks when combined with object parenting. In the simpler case where tables have the same corresponding rows in the same order, tables can be simply collated with each other.
